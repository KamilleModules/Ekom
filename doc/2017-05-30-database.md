...
This represents the ensemble of payment methods that a user has configured.
Note: it's a subset of the shop payment methods.


- user_id: fk
- shop_id: fk
- payment_method_id: fk
- options: text(serialized), the user config preferences for this payment method and this shop.
                            For instance, for the credit_card_wallet payment method, that could
                            be the id of the preferred card to use.
                            
- order: int, the lowest order is the preferred method payment for this shop


If you ask the question: which is the preferred payment method in a given set (A) of payment methods,
then the algorithm looks like this:

- take all entries in shop_has_payment_method, ordered by order asc, this is set C.  
- take all entries in user_has_payment_method, ordered by order asc, this is set B.
- foreach element of set B, if it's in set A and C, then it's the preferred element.
- otherwise, take the first element of set C that is in A.

In plain english, this means that the shop_has_payment method defines which payment methods
are available at all (i.e. the preferred payment method MUST be in that set).

Then, the user_has_payment_method defines the payment methods configured by the user.
If a user has configured a method, it always has precedence over an entry inside the shop_has_payment_method.
Even the least preferred method found in user_has_payment_method is preferred over 
the strongest shop_has_payment_method.
