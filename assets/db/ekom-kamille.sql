...
-- -----------------------------------------------------
-- Table `kamille`.`ek_tax_rule_has_tax`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `kamille`.`ek_tax_rule_has_tax` (
  `tax_rule_id` INT NOT NULL,
  `tax_id` INT NOT NULL,
  `cond_user_group_id` INT NULL,
  `cond_product_type_id` INT NULL,
  `cond_extra1` VARCHAR(64) NULL,
  `cond_extra2` VARCHAR(64) NULL,
  `cond_extra3` VARCHAR(64) NULL,
  `cond_extra4` VARCHAR(64) NULL,
  PRIMARY KEY (`tax_rule_id`, `tax_id`),
  INDEX `fk_ek_tax_rule_has_ek_tax_ek_tax1_idx` (`tax_id` ASC),
  INDEX `fk_ek_tax_rule_has_ek_tax_ek_tax_rule1_idx` (`tax_rule_id` ASC),
  INDEX `fk_ek_tax_rule_has_tax_ek_user_group1_idx` (`cond_user_group_id` ASC),
  INDEX `fk_ek_tax_rule_has_tax_ek_product_type1_idx` (`cond_product_type_id` ASC),
  CONSTRAINT `fk_ek_tax_rule_has_ek_tax_ek_tax_rule1`
    FOREIGN KEY (`tax_rule_id`)
    REFERENCES `kamille`.`ek_tax_rule` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ek_tax_rule_has_ek_tax_ek_tax1`
    FOREIGN KEY (`tax_id`)
    REFERENCES `kamille`.`ek_tax` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ek_tax_rule_has_tax_ek_user_group1`
    FOREIGN KEY (`cond_user_group_id`)
    REFERENCES `kamille`.`ek_user_group` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ek_tax_rule_has_tax_ek_product_type1`
    FOREIGN KEY (`cond_product_type_id`)
    REFERENCES `kamille`.`ek_product_type` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
