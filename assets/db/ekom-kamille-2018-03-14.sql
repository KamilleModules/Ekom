...
CREATE TABLE IF NOT EXISTS `kamille`.`ek_newsletter` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC),
  INDEX `fk_ek_newsletter_ek_shop1_idx` (`shop_id` ASC),
  CONSTRAINT `fk_ek_newsletter_ek_shop1`
    FOREIGN KEY (`shop_id`)
    REFERENCES `kamille`.`ek_shop` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `kamille`.`ek_cart` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_ek_cart_ek_user1_idx` (`user_id` ASC),
  INDEX `fk_ek_cart_ek_shop1_idx` (`shop_id` ASC),
  CONSTRAINT `fk_ek_cart_ek_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `kamille`.`ek_user` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ek_cart_ek_shop1`
    FOREIGN KEY (`shop_id`)
    REFERENCES `kamille`.`ek_shop` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
